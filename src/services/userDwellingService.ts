...
import { UserDwellingDAO } from '../daos/userDwellingDAO';
...
export class UserDwellingService {
    ...
    async saveUserDwelling(item: user_Dwellings) {
        try {
        
                console.log("In userdwelling service");
                console.log("In user dwelling" + item.user_id)
                let data = await this.userDwellingDAO.search({ user_id: item.user_id });
                if(data.length>0){
                        let returnData={
...
*/
  async getUserDwellings(){
      try{
          console.log("In getUserDwellings");
          let usersDwellingData = await this.userDwellingDAO.findAll();
          let returnData = {
              data : usersDwellingData,
              message: usersDwellingData.length
          }
          return Promise.resolve(returnData);
      }
   catch(error){
